...
export default class SplunkEvents {
  config(config) {
    ...
    this.injectAditionalInfo =
      config.injectAditionalInfo !== undefined
        ? config.injectAditionalInfo
        : false
    this.autoFlush = config.autoFlush !== undefined ? config.autoFlush : true
    this.autoRetryFlush =
      config.autoRetryFlush !== undefined ? config.autoRetryFlush : true
    this.source = config.source !== undefined ? config.source : 'log'
    this.path =
      config.path !== undefined ? config.path : '/services/collector/event'
  ...
  flush = () => {
    ...
    let splunkBatchedFormattedEvents = this.formatEventsForSplunkBatch(
      this.pendingEvents
    )
